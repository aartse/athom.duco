...
class CO2ValveDriver extends DucoDriver {
  async onInit() {
    ...
    const ventilationStateCondition = this.homey.flow.getConditionCard('co2-valve__ventilation_state_is');
    ...
    const ventilationModeCondition = this.homey.flow.getConditionCard('co2-valve__ventilation_mode_is');
  ...
  async onPairListDevices() {
    ...
    const nodes = (await DucoApiFactory.create(this.homey).getNodes()).filter(node => NodeHelper.isMappedForDriver(node.General.Type.Val, 'co2-valve'));
